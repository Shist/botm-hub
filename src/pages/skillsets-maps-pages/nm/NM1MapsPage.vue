<template>
  <div class="skillset-maps-page">
    <h2 class="skillset-maps-page__headline">Мапы NM1 (Raw Aim)</h2>
...
onMounted(async () => {
  try {
    await mapsStore.loadMapsByCategory(OsuMapCategory.nm1);
  } catch (error) {
    const msg = error instanceof Error ? error?.message : error;
    setErrorToast(
      `Не удалось загрузить карты для категории "${OsuMapCategory.nm1}": ${msg}`
    );
  }
});
...
const mapsInfo = computed(() => mapsStore.maps[OsuMapCategory.nm1]);
